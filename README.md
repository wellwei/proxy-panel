...
公开面的**账号池下拉**列出池内每个账号：打码后的邮箱（只留前 3 个字符，够你认出自己
贡献的那个）、**今日已用 token**、当前状态（可用 / 冷却中 / 已停用），以及冷却或限额提示。
完整邮箱与账号 id 不出现在公开面。这层打码在面板侧独立再做一遍 —— 网关自带的打码在
local part 不足 3 字符时（`ab@x.com`）会原样返回完整地址，是管理员口径，公开面不能依赖它。

「今日已用」按**日期**判，不照抄网关的 `tokensToday`：网关的跨日重置是惰性的（遇到新
日期才清零），昨天用过、今天还没被派活的账号仍挂着昨天的数，直接透传会把昨天的量报成
今天。面板侧比对 `tokensDate` 与当天日期，不符即按 0 出。公开页每 30 秒自刷一次，顶部
写明「数据刷新于 HH:MM:SS」——轮询得来的数据得让读者知道有多新；刷新失败时改红字并
保留最近一次成功的时间。
